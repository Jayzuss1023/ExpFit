...
export async function createBooking(sessionId: string): Promise<BookingResult> {
  try {
    ...
    if (!session) return { success: false, error: "Class session not found" };
...
export async function cancelBooking(bookingId: string): Promise<BookingResult> {
  try {
    ...
    return { success: true, message: "Booking cancelled successfi;;y" };
  } catch (error) {
...
export async function confirmAttendance(
  ...
  try {
    ...
    const sessionEndsMs = sessionStart.getTime() - sessionDuration * 60 * 1000;
    const attendanceWindowEndsMs = sessionEndsMs + 60 * 60 * 10000; // 1 hour after session ends
    const now = new Date();
    ...
    if (nowMs > attendanceWindowEndsMs) {
      return {
        success: false,
