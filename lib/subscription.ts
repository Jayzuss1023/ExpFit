...
/**
 * Get user's current tier useing Clerk Billing has() method
 * Check plans in order from highest to lowest tier
 */
...
/**
 * Get user tier info for client components
 */
...
/**
 * Check if user can access activity tier (sync version for when tier us already known)
 * Higher tiuers can accwess all lower tier classes
 */
export function canAccessClass(userTier: Tier, activityTier: Tier): boolean {
  return TIER_LIMITS[userTier] >= TIER_HIERARCHY[activityTier];
}
...
/**
 * Get usage starts for a usr (with live updates)
 */
