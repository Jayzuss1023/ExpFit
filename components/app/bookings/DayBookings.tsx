...
export function DayBookings({ bookings, selectedDate }: DayBookingsProps) {
  ...
  if (dayBookings.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center rounded-lg border bg-card p-6 text-center mr-4">
        ...
        <Link
          href="/classes"
          className="mt-3 text-sm font-medium text-teal-600 hover:text-teal-700 dark:text-teal-400"
        >
          Browse classes →
        </Link>
      </div>
        ...
        {dayBookings.map((booking) => {
          ...
          return (
            <Link
              ...
              className="group flex gap-4 rounded-lg border bg-card p-4 transition-colors hover:border-teal-500/50"
            >
              {/* Image */}
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <h4 className="truncate font-medium group-hover:text-teal-600">
