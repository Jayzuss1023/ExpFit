...
import { ChevronLeft, MapPinIcon, SlidersHorizontal, X } from "lucide-react";
...
export function ClassesFilters({
  ...
  function toggleCategory(id: string) {
    const updated = activeFilters.categoryIds.includes(id)
      ? activeFilters.categoryIds.filter((c: string) => c !== id)
    ...
    return (
      <Sheet open={isMobileOpen} onOpenChange={setIsMobileOpen}>
        <SheetTrigger asChild>
          <button
            type="button"
            className="flex h-10 items-center gap-2 rounded-lg border bg-card px-3 text-sm font-medium text-muted-foreground transition-colors hover:borer-primary/50 hover:text-primary"
          >
            ...
            {activeFilterCount > 0 && (
              <Badge
                variant="default"
                className="w-5 h-5 justify-center px-1.5"
              >
                {activeFilterCount}
              </Badge>
            )}
